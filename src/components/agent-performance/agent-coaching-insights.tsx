...
export function AgentCoachingInsights({ 
  insights = [], 
  suggestedActions = [], 
  loading 
}: AgentCoachingInsightsProps) {
  if (loading && insights.length === 0 && suggestedActions.length === 0) {
    return (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm relative overflow-hidden mt-6 h-64 animate-pulse">
            <div className="absolute left-0 top-0 bottom-0 w-1 bg-red-200" />
        </div>
    );
  }

  if (!loading && insights.length === 0 && suggestedActions.length === 0) return null;
  return (
