  ...
  const rows =
    data && data.length > 0
      ? data.map((r) => ({
          ...
          qualified:   Math.round(Number(r.qualified_pct) || 0),
          highIntent:  Number(r.high_intent) || 0,
          // API uses conversion_rate_pct; fallback to conversion_pct
          conversion:  Math.round(Number(r.conversion_rate_pct ?? r.conversion_pct) || 0),
          // API uses complaint_rate_pct; fallback to complaint_calls
          complaint:   Math.round(Number(r.complaint_rate_pct ?? r.complaint_calls) || 0),
          storeCount:  r.store_count != null ? String(r.store_count) : '—',
          alertUrgent: (Number(r.complaint_rate_pct ?? r.complaint_calls) || 0) > 30,
        }))
      : [
          { region: 'North', totalCalls: '4,200', qualified: 42, highIntent: 5,  conversion: 46, complaint: 17, storeCount: '6', alertUrgent: false },
          { region: 'South', totalCalls: '3,800', qualified: 50, highIntent: 13, conversion: 56, complaint: 17, storeCount: '9', alertUrgent: false },
          { region: 'East',  totalCalls: '3,500', qualified: 33, highIntent: 2,  conversion: 65, complaint: 33, storeCount: '3', alertUrgent: false },
          { region: 'West',  totalCalls: '3,200', qualified: 50, highIntent: 5,  conversion: 67, complaint: 43, storeCount: '7', alertUrgent: true  },
        ];
