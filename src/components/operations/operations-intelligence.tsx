...
export function OperationsIntelligence({ insights = [], actions = [], loading }: OperationsIntelligenceProps) {
  if (loading && insights.length === 0 && actions.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden relative w-full mb-6 h-[200px] animate-pulse">
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-red-200" />
      </div>
    );
  }

  // If no data and not loading, we can either hide or show empty
  if (!loading && insights.length === 0 && actions.length === 0) {
    return null; // Or show an "All systems optimal" message
  }
